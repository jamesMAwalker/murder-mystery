...
export const EndingCard = () => {
  ...
  const guiltySuspect = useQuery(api.suspects.getBySuspectId, {
    suspect_id:
      game?.guilty_suspect_id! ||
      ('55zgs7cfqt63krxvcccggc6e9jmqg38' as Id<'suspects'>)
  })
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 flex items-center justify-center p-2 bg-red-800 bg-opacity-50 backdrop-blur md:backdrop-blur-lg z-50',
        winningGuess ? 'bg-green-500' : 'bg-red-800'
      )}
    >
      <div className='w-full max-w-2xl bg-base-100 shadow-xl rounded-lg overflow-y-auto relative mx-2 md:mx-auto my-2 h-[85vh]'>
        <img
          src={guiltySuspect?.image_url}
          alt={guiltySuspect?.suspect_name}
